...
def best_first_graph_search(problem, f, display=False):
    ...
    total_removed_from_frontier = 0 #for the record of total number of nodes that were removed from the frontier
    ...
    while frontier:
        node = frontier.pop()
        total_removed_from_frontier += 1 #increment once the node removed
        
        if problem.goal_test(node.state):
            if display:
                print(len(explored), "paths have been expanded and", len(frontier), "paths remain in the frontier")
            return node, node.depth, total_removed_from_frontier #for the record of the length of the solution
...
def astar_search(problem, heuristic_specifier, display=False):    
    if heuristic_specifier == 'h_misplaced': #the misplaced tile heuristic
        h = memoize(problem.h_misplaced, 'h')
    elif heuristic_specifier == 'h_manhattan': #the Manhattan distance heuristic
        h = memoize(problem.h_manhattan, 'h')
    elif heuristic_specifier == 'h_max': #the max of the misplaced tile heuristic and the Manhattan distance heuristic
        h = memoize(problem.h_max, 'h')
...
class StagePuzzle(Problem):
    ...
    def h_misplaced(self, node): #the misplaced tile heuristic
        return sum(s != g for (s, g) in zip(node.state, self.goal))
    
    def h_manhattan(self, node): #the Manhattan distance heuristic
        return sum(abs(s - g) for (s, g) in zip(node.state, self.goal))
    
    def h_max(self, node): #the max of the misplaced tile heuristic and the Manhattan distance heuristic
